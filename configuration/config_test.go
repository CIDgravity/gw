...
func TestLoadConfig_Validation_LocalwebModes(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "reverse proxy mode with path prefix",
			url:       "https://fgw.example.com/cars",
			builtin:   "true",
			serverTLS: "false",
		},
		{
			name:      "reverse proxy mode with path prefix and trailing slash",
			url:       "https://fgw.example.com/cars/",
			builtin:   "true",
			serverTLS: "false",
		},
		{
			name:        "url path rejected in builtin autocert mode",
			url:         "https://fgw.example.com/cars",
			builtin:     "true",
			serverTLS:   "true",
			wantErr:     true,
			errContains: "must not include a path in built-in autocert mode",
		},
		{
			name:        "url path rejected for loopback builtin server",
			url:         "http://127.0.0.1:8443/cars",
			builtin:     "true",
			serverTLS:   "false",
			wantErr:     true,
			errContains: "serves CAR files at the host root",
		},
		{
